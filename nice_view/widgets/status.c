 ...
 *
 * Modified from ZMK v0.3 app/boards/shields/nice_view/widgets/status.c:
 *   - Removed WPM widget (zmk/wpm.h, wpm_state_changed event, wpm drawing code).
 *   - Added right-side (peripheral) battery indicator in the space WPM occupied.
...
struct right_battery_status_state {
    uint8_t level;
};
...
static void draw_top(lv_obj_t *widget, lv_color_t cbuf[], const struct status_state *state) {
    ...
    // Draw left battery bar (top-left area, pre-rotation)
    draw_battery(canvas, state);

    // Draw output/BT status icon (top-right area, pre-rotation)
    char output_text[10] = {};
...
// ── Right (peripheral) battery ───────────────────────────────────────────────

static void set_right_battery_status(struct zmk_widget_status *widget,
                                     struct right_battery_status_state state) {
    widget->state.right_battery = state.level;
    draw_top(widget->obj, widget->cbuf, &widget->state);
}
...
static struct right_battery_status_state right_battery_status_get_state(const zmk_event_t *eh) {
    const struct zmk_peripheral_battery_state_changed *ev =
        as_zmk_peripheral_battery_state_changed(eh);

    if (ev != NULL) {
        return (struct right_battery_status_state){.level = ev->state_of_charge};
    }

    uint8_t level = 0;
    zmk_split_central_get_peripheral_battery_level(0, &level);
    return (struct right_battery_status_state){.level = level};
}

ZMK_DISPLAY_WIDGET_LISTENER(widget_right_battery_status, struct right_battery_status_state,
                            right_battery_status_update_cb, right_battery_status_get_state)

ZMK_SUBSCRIPTION(widget_right_battery_status, zmk_peripheral_battery_state_changed);
...
int zmk_widget_status_init(struct zmk_widget_status *widget, lv_obj_t *parent) {
    ...
    widget_right_battery_status_init();
